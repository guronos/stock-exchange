<template>
     <div>
        <v-table density="compact" class="mt-12 bg-grey-lighten-5 rounded-lg elevation-10">
            ...
            <caption class="bg-blue-grey-darken-2">Биржевой стакан {{ store.getters.showTikers }}</caption>
            <thead class="bg-blue-grey-darken-2"><th>Цена<v-divider /></th>
            <th>Количество<v-divider /></th>
            <th>Всего<v-divider /></th>    
        </thead>
            <tbody class="text-body-2">
                <AllOrders 
            :colorClass="'text-deep-orange-darken-3'"
            :orders="asks"
            :userOrders="userOrdersAsks"
            />
            ...
            <AllOrders 
            :colorClass="'text-green-accent-4'"
            :orders="bids"
            :userOrders="userOrdersBids"
            /></tbody>
        </v-table>
    </div>
</template>
